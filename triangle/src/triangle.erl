...
-type triangle_form() :: equilateral 
                         | isosceles 
                         | scalene.

-spec triangle_inequality([integer()]) -> 
                    atom().
triangle_inequality([A, B, C]) 
        when (C < (A + B)) -> 
        valid_triangle;
triangle_inequality([_, _, _]) -> 
        violated_triangle_inequality.

-spec is_valid(number(), number(), number()) ->     
                    atom().
is_valid(A, B, C) when ((A =< 0) 
                        or (B =< 0) 
                        or (C =< 0)) -> 
                not_all_sides_positive;
is_valid(A, B, C) -> 
                triangle_inequality(lists:sort([A, B, C])).
