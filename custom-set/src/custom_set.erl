...
-export([ add/2, contains/2, difference/2, disjoint/2, empty/1, 
		  equal/2, from_list/1, intersection/2, subset/2, union/2 ]).
...
contains(_, Set) when Set#custom_set.elements == [] -> false;
contains(Elem, Set) -> #custom_set{elements = Elements} = Set,
		[H|T] = Elements,
		case H of
			Elem -> true;
...
intersection(Set1, Set2) -> #custom_set{elements = Elements1} = Set1,
							#custom_set{elements = Elements2} = Set2,
							#custom_set{elements = lists:filter(fun(Elem) -> 
					 								contains(Elem, 
...
dedup(List) -> ordsets:from_list(ordsets:to_list(List)).